...
func (s *svc) getPostsPaginated(ctx context.Context, search string, limit, offset int, requestingUserID *pgtype.UUID) ([]PostCardDTO, error) {
	...
	if err != nil {
		return []PostCardDTO{}, fmt.Errorf("failed to fetch posts: %s", err.Error())
	}
